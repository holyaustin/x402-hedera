...
This demo intentionally sells **two products priced in native HBAR** and
**one priced in USDC**, on the same server, to show the resource server can
accept either asset. Check `server/src/products.ts` — each entry in
`CATALOG` has an `asset: "HBAR" | "USDC"` field. `server/src/index.ts` reads
that field and routes each product's payment to the right asset via a
money parser registered on the Hedera scheme (the officially sanctioned way
to price in something other than a chain's default asset — see the comment
block at the top of `index.ts` for the full reasoning).
...
If you ever see `Cannot find module 'hono'` or `Cannot find module
'@hiero-ledger/sdk'` **in your editor** but the server actually runs fine
from the terminal, that's a stale TypeScript language server cache, not a
real problem. Restart it: in VS Code, Cmd/Ctrl+Shift+P → "TypeScript:
Restart TS Server". If the terminal itself also can't find the modules, do
a genuinely clean reinstall:
...
**For the two HBAR-priced products** (`spot-price`, `ai-insight`), you're
already good to go — every Hedera testnet account is funded with HBAR the
moment it's created.

**For the USDC-priced product** (`quote`), you need two extra one-time
steps: associate your buyer account with the USDC token, then fund it from
Circle's testnet faucet.

```bash
cd server
npm run associate          # associates your buyer account with testnet USDC
```

Then go to **https://faucet.circle.com**, select **Hedera Testnet**, and
paste your buyer account ID (`0.0.xxxxx`) to receive testnet USDC.

Now, with the server still running, in a second terminal:
...
transaction id. That id is what you'll look up on HashScan later. The
server's own console log also prints which asset each product settles in
when it starts up.
